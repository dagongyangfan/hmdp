...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     */
    @Override
    public Result sendCodeSession(String phone, HttpSession session) {
        // 1.校验手机号(正则式)
    ...
    /**
     * 使用Redis替代Session完成短信登录验证
     * @param phone
     * @param session
     * @return
     */
    @Override
    public Result sendCodeRedis(String phone, HttpSession session) {
        // 1.校验手机号(正则式)
        if (RegexUtils.isPhoneInvalid(phone)) {
            // 2.如果不符合，返回错误信息
        ...
        /* 4.保存验证码到 redis，并设置验证码有效期为常亮LOGIN_CODE_TTL，单位：分钟
        *  key = 前缀 + phone，保证每一个电话号码对应唯一一个key
        * */
    ...
    /**
     * Redis替代Session：从Redis中获取验证码完成登录校验
     * @param loginForm
     * @param session
     * @return
     */
    @Override
    public Result loginRedis(LoginFormDTO loginForm, HttpSession session) {
        // 1.校验手机号是否合法
        String phone = loginForm.getPhone();
        if (RegexUtils.isPhoneInvalid(phone)) {
            // 2.如果不符合，返回错误信息
